...
if(isset($userData['acc_name'])){
    $accname = $userData['acc_name'];
}
else{
    $accname = '';
}
...
if(isset($userData['acc_email'])){
    $userData = $userData['acc_email'];
}
$uEmail     =   $userData;
...
$token
    ...
    <div class="navbar-inverse bg-black" style="color:#fff">$senderInfoT</div>
        ...
        <fieldset class="sender_countryFieldset">
           <input type='text' class="form_field" id="sender_country" name="sender_country" value='$storeCountry' class=''/>
        </fieldset>
        </div><!--form_fill end--> 
    ...
    <div class='button_area fadeInLeft' >
        <button type="submit" onclick="return finalFormSubmit();" name="submit" value="ORDER" class="submit btn-success btn themeButton btn-lg">
            $orderT
        </button>
    </div><!--btn_area end-->
