...
var ajax1 = $('.dTable_ajax1').each(function(){
        url  = $(this).attr("data-href");
        uniq = $(this).attr("data-uniq");
        <?php #### ////// columnDefs is used in columnDefs of dataTable. ?>
        var columnDefs;
        ...
        OrdersTable = $(this).DataTable({
            <?php //echo $aLengthMenu; ?>
            // Show Entries Values
            "aLengthMenu": [[5, 10, 25, 50, 100, 250, 500, 1000, 5000, 10000, -1],[5, 10, 25, 50, 100, 250, 500, 1000, 5000, 10000, "All"]],
            "sDom": 'T C<"clearfix">lfrtip',
            "bSort": bSortValue, // this will disable sorting all around, "orderable" inside "columnDefs" won't work 
            "columnDefs": columnDefs,
            <?php echo $oTableTools; ?>
...
// ajax1.column( 3 ).data().sum();
    ...
    dTableRangeSearch =function(){$.fn.dataTable.ext.search.push(
        ...
        $('#min, #max').change( function() {
            ...
            // console.log('main.php ,min ,max change event occurs ');
        });
    };
...
//use for count animation End
    ...
    notification = function(heading,text,clas){
        ...
        notifi = '<div id="noti_'+uniqueNum+'" class="notification_side2">'+
                    '<div id="noti_'+uniqueNum+'" class="notification_side2_box check_positive check_positive_add '+clas+'">'+
                        '<div class="notification_side2_box_txt"> '+text+' </div>'+
                        '<div class="notification_side2_box_close_btn">&times;</div>'+
                    '</div>'+
                '</div>';

        $('.notification_side2').prepend(notifi);
        $('#noti_'+uniqueNum).hide().fadeIn('slow');
        notificationAutoRemove(uniqueNum,10);
    };

    textNotification = function(text,clas){
        var uniqueNum = Math.floor( Math.random()*99999 );

        notifi = '<div id="noti_'+uniqueNum+'" class="page_notification_box '+clas+'"> '+text+' </div>';

        $('.page_notification').prepend(notifi);
    };

    alertBoxNotification = function(heading, message, clas, size){

        if(size == 'small'){
            alertSize = '';
        }else if(size == 'big'){
            alertSize = 'increase_size';
        }

        notifi = '<div class="alert_popup '+alertSize+' '+clas+' alert_popup_add">'+
                    '<div class="alert_popup_txt">'+
                        '<h3>'+heading+'</h3>'+
                        '<div class="alert_popup_txt_box">'+message+'</div>'+
                    '</div>'+
                    '<div class="notification_side2_box_close_btn">×</div>'+
                '</div>';

        $('.col_right').prepend(notifi);
    };
