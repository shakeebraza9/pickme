...
class webUsers extends object_class
{
    ...
    public function webUsersView()
    {
        ...
        if ($this->functions->developer_setting('cartSystem') == '1' && $this->functions->developer_setting('product') == '1') {
            $product = true;
            echo "<th>" . _u($_e['ITEMS IN CART']) . "</th>";
            ...
            try {
                ...
                if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    ...
                    $sql = "UPDATE  accounts_user SET
                                ...
                                user_typeee = ?
                                WHERE acc_id = '$id'";
                    $array = array($name, $email, $type, $accountType);
                    $this->dbF->setRow($sql, $array, false);
                ...
                $this->db->commit();
                if ($this->dbF->rowCount > 0) {
                    // $accountType   = empty($_POST['accountType']) ? 0 : $_POST['accountType'];
    ...
    public function webUserInfoArray2($id)
    {
        if (!empty($id)) {
            $qry = "SELECT `user_typeee` FROM `accounts_user` Where `acc_id`='$id'";
    ...
    public function webUserEdit($id = '', $new = false)
    {
        ...
        echo '<div class="form-group">
                ...
                <div class="form-group">
                    <label class="col-sm-2 control-label">' . $_e['Account Type'] . '</label>

                    <div class="col-sm-10">
                        <label class="radio-inline">
                            <input type="radio" class="accountType" name="accountType" value="Client">' . strtolower($_e['User']) . '
                        </label>
                        <label class="radio-inline">
                            <input type="radio" class="accountType" name="accountType" value="Editor">' . $_e['editor'] . '
                        </label>
                    </div>
                </div>
                <script>
                $(document).ready(function(){
                    $(".accountType[value=\'' . $this->webUserInfoArray2(@$_GET['userId']) . '\']").attr("checked", true);
                });
...
<hr>
    ...
    public function adminUserEditSubmit()
    {
        ...
        if (
            ...
            if (!$this->functions->getFormToken('adminUserEdit')) {
                return false;
            }
            try {
    ...
    public function adminUserAddSubmit()
    {
        ...
        if (
            ...
            if (!$this->functions->getFormToken('adminUserNew')) {
                return false;
            }
            try {
    ...
    public function adminGrpSubmit()
    {
        ...
        if (
            ...
            if (!$this->functions->getFormToken('adminGrpNew')) {
                return false;
            }
            $name   =   $_POST['name'];
    ...
    public function adminGrpEditSubmit()
    {
        ...
        if (
            ...
            if (!$this->functions->getFormToken('adminGrpEdit')) {
                return false;
            }
