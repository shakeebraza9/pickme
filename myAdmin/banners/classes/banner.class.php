...
$_w['Video Recommended Size : {{size}}'] = '' ;
...
echo'<td><video style="max-height: 140px;;max-with:500px">
  <source src="'.$layer0.'">
...
`banner_link`, `banner_heading`, `banner_shrtDesc`,`layer0`,`layer1`,`layer2`,`layer3`,`category`,`publish`)
...
$heading        = empty($_POST['banner_heading'])   ? ""    : serialize($_POST['banner_heading']);
    ...
    $link        = empty($_POST['banner_link'])   ? ""    : serialize($this->functions->removeWebUrlFromLink($_POST['banner_link'])); 
...
<video controls class="layer0 kcFinderVideo">
  <source src="'.$image0.'" >
  Your browser does not support the video tag.
</video>
...
<label class="col-sm-2 col-md-3  control-label">'. _replace('{{size}}',$size,$_e['Video Recommended Size : {{size}}']) .'</label>
...
<div class="input-group-addon pointer " onclick="'."openKCFinderVideoWithVideo('layer0')".'"><i class="glyphicon glyphicon-facetime-video"></i></div>
