...
HTML;
    if ($product->editPid != "") {
        ...
        $editImageSize        = $editData['image_size'];
    ...
    for ($i = 0; $i < sizeof($lang); $i++) {
        ...
        if ($isEdit) {
            ...
            $eName = $eImage = $eTag = $eShortDesc = $eLongDesc = $eEditFeature = $eEditFeatureP = $specification = $size_chart = $category = "";
        }
...
<div class="panel-body">';
        ...
        $form_fields[] = array(
            'label' => _uc($_e['images size']),
            'name' => "$product->prefix_productBasicInformation[images][$lang[$i]]",
            'placeholder' => _uc($_e['images size']),
            'value' => "$eImage",
            'type' => 'number',
            'class' => 'form-control'
        );
...
$form_fields[] = array(
    "label" => $_e['Package Type'],
    'type' => 'checkbox',
    'value' => "$valFormTemp",
    'select' => "1",
    'format' => '<div class="make-switch" data-off="warning" data-on="success" data-on-label="' . _uc($_e['Edit']) . '" data-off-label="' . _uc($_e['Without']) . '">

{{form}}

<input type="hidden" name="' . $product->prefix_setting . '[Packagetype]" class="checkboxHidden" value="' . $val . '" />
...
<input type="hidden" id="AjaxFileNewPage" value="shopproduct">
