            ...
            $qry    =   "SELECT `proudct_detail_spb`.*, `product_setting_spb`.`setting_val`
                         FROM
                            `proudct_detail_spb` join `product_setting_spb`
                             ...
                             WHERE `product_setting_spb`.`setting_name`='publicAccess' AND `product_setting_spb`.`setting_val`='1'
                             AND `proudct_detail_spb`.`product_update`='1'
                             AND `proudct_detail_spb`.`prodet_id` IN (SELECT procat_prodet_id FROM product_category_spb WHERE `product_category_spb`.`procat_cat_id` LIKE '%$cat%' )
                             ";
