...
class discount extends object_class
{
    ...
    public function discountSettingData($pId){
        $sql ="SELECT `product_discount_spb`.*, `product_discount_setting_spb`.* FROM
                    `product_discount_spb` join `product_discount_setting_spb`
                    on `product_discount_spb`.`product_discount_pk` = `product_discount_setting_spb`.`product_dis_id`
                    WHERE `product_discount_spb`.`discount_PId`    = '$pId' ";
    ...
    public function discountPriceData($pId){
         $sql ="SELECT `product_discount_spb`.*, `product_discount_prices_spb`.* FROM
                `product_discount_spb` join `product_discount_prices_spb`
                on `product_discount_spb`.`product_discount_pk` = `product_discount_prices_spb`.`product_dis_id`
                WHERE `product_discount_spb`.`discount_PId`    = '$pId' ";
        $data = $this->dbF->getRows($sql);
        if($this->dbF->rowCount>0){
            return $data;
                ...
                FROM
                   `proudct_detail_spb` join `product_setting_spb`
                    ...
                    WHERE `product_setting_spb`.`setting_name`='publicAccess'
                      ...
                      AND `proudct_detail_spb`.`prodet_id` in
                        (SELECT discount_PId FROM product_discount WHERE product_dis_status = '1'
                          AND `product_discount_pk` in
                                ...
                                product_dis_id in (SELECT product_dis_id FROM `product_discount_setting_spb` WHERE `product_dis_name` ='dateFrom' AND `product_dis_value` <= '$today') AND
                                product_dis_id in (SELECT product_dis_id FROM `product_discount_setting_spb` WHERE `product_dis_name` ='dateTo' AND (`product_dis_value` >= '$today' OR `product_dis_value` = '') )) ) ";
                ...
                FROM
                   `proudct_detail_spb` join `product_setting_spb`
                    ...
                    WHERE `product_setting_spb`.`setting_name`='publicAccess'
                      AND `product_setting_spb`.`setting_val`='1'
                      AND `proudct_detail_spb`.`product_update`='1'
                      AND `proudct_detail_spb`.`prodet_id` in (SELECT discount_PId FROM product_discount WHERE product_dis_status = '0') ";
                      ...
                      AND `proudct_detail_spb`.`prodet_id` in
                        (SELECT discount_PId FROM product_discount WHERE product_dis_status = '1'
                          AND `product_discount_pk` in
                                (SELECT product_dis_id FROM product_discount_setting WHERE
                                product_dis_id in (SELECT product_dis_id FROM `product_discount_setting_spb` WHERE `product_dis_name` ='dateFrom' AND `product_dis_value` > '$today' ) AND
                                product_dis_id in (SELECT product_dis_id FROM `product_discount_setting_spb` WHERE `product_dis_name` ='dateTo' AND `product_dis_value` > '$today' AND `product_dis_value` != '')) ) ";
                ...
                echo "
                        <tr class='p_$discountId'>
                            ...
                            <td class='tableBgGray' width='110'>
                                ...
                                <a href='-product_spb?page=pDiscountForm&pId=$id' class='btn' target='_blank'><i class='glyphicon glyphicon-edit'></i></a>
