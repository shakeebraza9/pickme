...
function addcoupon(){
        ...
        //    if(!$functions->getFormToken('saleForm')){ return false;}
            try{
                ...
                if($_POST['edit']!=''){
                    ...
                    $sql ="DELETE FROM `product_coupon_prices_spb` WHERE pSale_price_id = '$editId'";
                    $dbF->setRow($sql);

                    $sql ="DELETE FROM `product_coupon_setting_spb` WHERE pCoupon_id = '$editId'";
                    $dbF->setRow($sql);

                    $sql    = "UPDATE `product_coupon_spb` SET
                                `pCoupon_name`=?,
                                `pCoupon_from`=?,
                                `pCoupon_to`=?,
                                `pCoupon_status`=?,
                                ...
                                `pCoupon_products`=?,
                                `pCoupon_country`=?,
                                `pCoupon_type`=?,`pCoupon_limit`=?,
                                `pCoupon_format`=?
                                
                               WHERE pCoupon_pk = '$editId'
                              ";
                }else{
                    $sql    = "INSERT INTO `product_coupon_spb`
                                   (`pCoupon_name`,`pCoupon_from`, `pCoupon_to`,
                                      `pCoupon_status`, `pCoupon_discount`, `pCoupon_category`, `pCoupon_products`,
                                      `pCoupon_country`,`pCoupon_type`,`pCoupon_limit`,`pCoupon_format`)
                                    VALUES
                                     (?,?,?,?,?,?,?,?,?,?,?)";
                }
                ...
                $coupon_type       =  empty( $_POST['coupon_type'])  ? ""    :  $_POST['coupon_type'];
                $sale_name  =   empty( $_POST['sale_name'])          ? ""    :  $_POST['sale_name'];
                $pCoupon_limit  =   empty( $_POST['pCoupon_limit'])          ? ""    :  $_POST['pCoupon_limit'];


                $discountPPv   =   '';

                    @$discountPP = empty($_POST['discountPP'])?array():$_POST['discountPP'];
                    foreach($discountPP as $discountPPK){
                        $discountPPv .= $discountPPK.",";
                    }
                    $discountPPv = trim($discountPPv,',');
              ...
              //   echo "pCoupon_limit======".$pCoupon_limit;
               //  echo "discountFormat=====".$discountFormat;




                $array      =   array($sale_name,$dateFrom,$dateTo,$status,$discount,$cats,$discountPPv,$shipping,$coupon_type,$pCoupon_limit,$discountFormat);
 ...
 // var_dump($lastId);
                //Discount Setting
                $sql ="";
                $sql    =   "INSERT INTO `product_coupon_setting_spb`
                ...
                //Discount Prices
                $sql ="";
                $sql ="INSERT INTO `product_coupon_prices_spb`
...
<?php } ?>
        ...
        <div class="discountForm">
            <form action="" method="post" class="form-horizontal">
             ...
             <div class="form-group">
                    <label class="col-sm-3 col-md-2 control-label" for="pCoupon_limit"><?php echo _uc($_e['Enter Limit']); ?></label>
                    <div class="col-sm-9 col-md-10 form-inline">
                        ...
                        $Limit     =    '';
                        if($editDiscount){
                            @$limit = $discountSettingData[0]['pCoupon_limit'];
                        }
                        ?>
                        <input name="pCoupon_limit" id="pCoupon_limit" class="form-control" value="<?php echo  @$limit; ?>" placeholder="<?php echo _uc($_e['Enter Limit']); ?>" required="true">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9 col-md-10">
                        ...
                        <select name="discount[discountFormat]" id="discountFormat" class="form-control to">
                            <option value="price"><?php echo _uc($_e['In Price']); ?> (No VAT Added)</option>
                            <option value="percent"><?php echo _uc($_e['In Percent %']); ?> (VAT Will Be Added)</option>
...
<div class="form-group">
<label class="col-sm-3 col-md-2 control-label">Select Products</label>
<div class="col-sm-9 col-md-10">

<?php  echo '<select name="discountPP[]" class="form-control selects" id="example-getting-started1" style="height:300px" multiple="">';
 $pCoupon_products = $discountSettingData[0]['pCoupon_products'];


$people =explode(",",$pCoupon_products);
// var_dump($pCoupon_products);
$sqls = "SELECT * FROM `proudct_detail_spb` WHERE `validity`>1 AND `prodet_id` IN(SELECT `p_id` FROM `product_setting_spb` WHERE setting_name='publicAccess' AND setting_val!=0 ) ORDER BY `proudct_detail_spb`.`prodet_id` ASC";

 $product_array  =  $dbF->getRows($sqls);

// in_array(needle, haystack)

foreach ($product_array as $key => $value) {
$tmpID=$value['prodet_id'];
 $sqlp = "SELECT `propri_price` FROM `product_price_spb` WHERE `propri_prodet_id`='$tmpID'";
 $productPrice  =  $dbF->getRows($sqlp);


echo @$selectrelate = (in_array(@$value['prodet_id'], @$people))? 'selected' : '';
...
$smlImg1 = '';
echo '<option data-img="'.$smlImg1.'" value="'.$value['prodet_id'].'" '.@$selectrelate.'>'.translateFromSerialize($value['prodet_name']).' - '.$productPrice[0]['propri_price'].'GBP - '.$value['category'].' - Validity: = '.$value['validity'].'</option>';
}
                    ...
                    <script type="text/javascript">
                        $(document).ready(function(){
                            $("#tree").jstree({
                                'core': {
                                    'data': {
                                        'url': '<?php echo WEB_URL; ?>/<?php echo ADMIN_FOLDER; ?>/product_management_spb/?operation=get_node',
                                        'data': function (node) {
                                            return { 'id': node.id };
                                        }
                                    }
