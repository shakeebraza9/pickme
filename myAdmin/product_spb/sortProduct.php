    ...
    <h2 class="borderIfNotabs sub_heading"><?php echo _uc($_e['Sort Products']); ?></h2>
        ...
        if(!isset($_GET['cat'])) {
            ...
            foreach ($catId as $val) {
                ...
                $cat_like .= " `product_category_spb`.`procat_cat_id` LIKE '%$cId%' OR";
            ...
            $qry    =   "SELECT `proudct_detail_spb`.* FROM

                            `proudct_detail_spb` join `product_setting_spb`

                             on `proudct_detail_spb`.`prodet_id` = `product_setting_spb`.`p_id`

                             WHERE `proudct_detail_spb`.`product_update`='1'

                              AND `proudct_detail_spb`.`prodet_id` IN (SELECT procat_prodet_id FROM product_category_spb WHERE $cat_like GROUP BY procat_prodet_id)

                              GROUP BY `proudct_detail_spb`.`prodet_id` ORDER BY proudct_detail_spb.sort";
        ...
        $( "#sortProducts" ).sortable({
            ...
            update : function () {
                ...
                $.ajax({
                    ...
                    url: 'product_management_spb/product_ajax.php?page=sortProducts',
