...
$admin_email = $this->functions->send_mail($adminMail,"","","serviceComplete",$cname,$mailArray, false);
    ...
    public function cancelAgreement($orderId=false){
        ...
        $paymentType  = $this->functions->ibms_setting('Email');
        ...
        if($paymentType == 'stripe'){
                ...
                echo '1';
                    ...
                    if($mandate !== 'oneOffPayment'){
                        $sql = "UPDATE `orders` SET `order_status` = 'cancelled' WHERE `order_id` = ?";
                        $res = $this->dbF->setRow($sql, array($order_id));
                        $sql_invoice = "UPDATE `invoices` SET `invoice_status` = 'cancelled' WHERE `order_id` = ? AND `invoice_status` = 'pending'";  
                        $res_invoice = $this->dbF->setRow($sql_invoice, array($order_id));
                        
                        $stripeSecret = $this->getStripeSceret();
                        $str_res =  deletePaymentMethod("pm_1NCLbnARYcQcurTJel7JTEsb" , $stripeSecret); 
