...
<script src="<?php echo WEB_ADMIN_URL; ?>/order_spb/js/jSignature.js"></script>
<script src="<?php echo WEB_ADMIN_URL; ?>/order_spb/js/plugins/jSignature.CompressorSVG.js"></script>
<script src="<?php echo WEB_ADMIN_URL; ?>/order_spb/js/plugins/jSignature.UndoButton.js"></script>
...
<div class="table-responsive newProduct col-sm-12">
	...
	<th colspan="7">
		...
		<tr>
			<td class="gray-tr"><?php echo _uc('Mandate'); ?></td>
			<td><?php echo $data['order_mandate']; ?></td>
		</tr>
		<tr>
			<td class="gray-tr"><?php echo _uc('Customer'); ?></td>
			<td><?php echo $data['order_customer']; ?></td>
		</tr>
	</table>
</div>
<!-- sender detail end -->
...
<div class="table-responsive newProduct">
	...
	<th colspan="12">
		...
		<th><?php echo _u('TOTAL RECEIVED'); ?></th>
		</tr>
		<?php 
		...
		<td><?php echo 'GBP '.$data['price_per_month']; ?></td>
		<td><?php echo _uc($admin_order_status); ?></td>
		<td><?php echo 'GBP '.$cumulative_total; ?></td>
		</tr>
	</table>
</div>
<!-- product detail end -->
...
<?php
$sql = "SELECT * FROM `invoices` WHERE `order_id` = ? AND `invoice_status` IN ('paid','pending_submission','submitted') ORDER BY `due_date` ASC";
...
$sqls = "SELECT `order_customer` FROM `orders` WHERE order_id='$value[order_id]'";
...
<div class="clearfix"></div>
<br>
<form class="form-horizontal schedule_form" method="POST">
<?php //$functions->setFormToken('ndateinvoice'); ?>
<div class="form-group">
<label for="full_name" class="col-sm-2 col-md-3 control-label"> No.</label>
<div class="col-sm-10 col-md-3">
<input type="hidden" name="" class="orderId" value="<?php echo $orderId; ?>"/>
<input type="text" name="ndate" class="form-control ndate" required="" id="" placeholder="Invoice next">
</div>


<div class="col-sm-10 col-md-3">
<input type="button" id="" name="" value="Next Invoice Generate" class="btn btn-success btn-lg generatenv">
</div>
</div>
...
<input type="button" id="cancel_agreement" name="cancel_agreement" value="CANCEL AGREEMENT"
...
$(".generatenv").on('click', function(event) {
  event.preventDefault();
    ...
    $.ajax({
        type: 'post',
        data: {ndate:ndate,orderId:orderId},
      url: 'order_spb/order_ajax.php?page=ndateinvoice',

    }).done(function(data) {
        if (data) {
             

        jAlertifyAlert('Invoice is Successfully Generated!');
...
// $.ajax({
// url: 'order_spb/order_ajax.php?page=submitSceduleForm',
// type: 'post',
// data: form
// }).done(function(res){
// console.log(res);
...
// $.ajax({
// url: 'order_spb/order_ajax.php?page=openTechnicalForm',
// type: 'post',
...
// $.ajax({
// url: 'order_spb/order_ajax.php?page=viewTechnicalForm',
// type: 'post',
...
$.ajax({
url: 'order_spb/order_ajax.php?page=cancelAgreement',
type: 'post',
...
$.ajax({
type: 'POST',
url: 'order_spb/order_ajax.php?page=deldeleteScheduleForm&id='+id,
data: { itemId:id }
}).done(function(data)
{
ift =true;
...
$.ajax({
url: 'order_spb/order_ajax.php?page=editSchedule',
type: 'post',
data: {id:id}
}).done(function(res){
...
$.ajax({
url: 'order_spb/order_ajax.php?page=submitEditSchedule',
type: 'post',
data: edit_form
}).done(function(res){
if(res == '1'){
...
$.ajax({
url: 'order_spb/order_ajax.php?page=updateInvoice',
type: 'post',
data: {id:id}
}).done(function(){
