...
$sql        =  "SELECT id FROM rating WHERE (user_id = '$data[user_id]' OR ip='$data[ip]') AND p_id = '$data[p_id]' AND type='price'";
...
$sql        =  "SELECT id FROM rating WHERE (user_id = '$data[user_id]' OR ip='$data[ip]') AND p_id = '$data[p_id]' AND type='quality'";
$dataOld    =  $this->dbF->getRow($sql);
if(empty($dataOld)){
$lastId = $this->functions->formInsert("rating",$data);
}else{
...
$sql        =  "SELECT id FROM rating WHERE (user_id = '$data[user_id]' OR ip='$data[ip]') AND p_id = '$data[p_id]' AND type='value'";
$dataOld    =  $this->dbF->getRow($sql);
if(empty($dataOld)){
$lastId = $this->functions->formInsert("rating",$data);
}else{
...
public function ratingInfo($id,$type= false,$u_id= false){
...
$sql    =   "SELECT count(id) as cnt,avg(rate) as rate FROM rating WHERE p_id='$id'";
...
global $_e;
$sql    =   "SELECT count(id) as cnt,avg(rate) as rate FROM rating WHERE p_id='$id' AND type='value' AND type='price' AND type='quality'";
$data   =   $this->dbF->getRow($sql);
$avg    =   round(floatval($data['rate']),1);
$full   =   floor($avg);
$ceil   =   ceil($avg);
...
return "<div class='rate_widget rating_type_value' data-type='value' data-pId='$id'>
$msg
$ratings
...
<div class='rate_widget rating_type_price' data-type='price' data-pId='$id' style='display:none'>
$msg
$ratings
<div class='total_votes'>$votes</div>
</div>
...
<div class='rate_widget rating_type_quality' data-type='quality' data-pId='$id' style='display:none'>
$msg
$ratings
<div class='total_votes'>$votes</div>
</div>
