...
trait common_function
{
    ...
    public function is_owner_admin(){
        if( isset($_SESSION['_roleGrp']) && $_SESSION['_roleGrp']=='0' &&
    ...
    */
    ...
    public function  resizeImage($image,$width='auto',$height='auto',$echo = true,$pngBgColor=false,$imageWithWebUrl=true,$cache=true){
        ...
        if($width   !='auto' || $width  !=''){  $resize .= "&width=".$width;}
        if($height  !='auto' || $height !=''){  $resize .= "&height=".$height; }

        if($cache===false){         $resize .= "&nocache=nocache";}
        if($pngBgColor!==false){    $resize .= "&color=".$pngBgColor;}

        if($imageWithWebUrl){   $image = WEB_URL.'/images/'.$image;}
...
<div class="imedia_tag" id="a_<?php echo $logoId ?>">
            <div class="align">
                <div class="i_media">
                    <a href="http://imedia.com.pk/" title="International Web Development Company" class="interactive" target="_blank"><img src="webImages/imedia.png" alt="Professional Website Development"></a>

                    <div class="i_text"><a href="http://imediaintl.com/" target="_blank" title="Web Designing Company In Europe" >Design</a> &amp; <a href="http://imediadf.com/" title="International eStore Development Company" target="_blank">Developed</a>
                        By <a href="http://imedia.com.pk/" title="International Web Development Company" class="interactive" target="_blank">Interactive Media</a></div>
                </div>
            </div>
        </div>
    <?php }
