...
$comment
<blockquote class='small padding-F10 margin-5'>($date)</blockquote>
...
<label class='col-sm-2 control-label'>". _uc($_e['Name']) ." </label>
<div class='col-sm-10'>
...
$sql    =   "SELECT * FROM `reviews` WHERE r_id = '$id' AND `type` = '$type' AND (status = '1') ORDER BY id $reviewOrderBY $limit";
$data   =   $this->dbF->getRows($sql);

//count comments
$sql    =   "SELECT COUNT(id) as id FROM `reviews` WHERE r_id = '$id' AND `type` = '$type' AND (status = '1')";
$data2   =   $this->dbF->getRow($sql);
$count  =   $data2['id'];
...
$rating     = new rating();
$rate = $rating->ratingInfo($id,FALSE,$val['user_id']);
...
$temp .= "<div class='form-group'>
<label class='col-sm-2 control-label'>". _uc($_e['Add Rating']) ." </label>
<div class='col-sm-10'>

". $rating->ratingView($id,'value',"") ."
