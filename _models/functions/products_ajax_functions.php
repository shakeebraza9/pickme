...
public function packageinfo(){
        ...
        $productId=$_POST['productId'];    
         ...
         $sql = "SELECT * FROM `proudct_detail_spb` WHERE `product_update`='1' AND prodet_id=$productId AND `category`='Month'";
        $data = $this->dbF->getRow($sql);

     $prodet_id = $data['prodet_id'];
    $validity = intval($data['validity']);
    // var_dump($prodet_id);
    $productName = unserialize($data['prodet_name']);
    $productKey=key($productName);
    $name = $productName[$productKey];
    $sql2 = "SELECT `propri_price` FROM `product_price_spb` WHERE `propri_prodet_id`=$prodet_id";
    $data2 = $this->dbF->getRow($sql2);
    if($data2){
    $price=$data2['propri_price'];
        
    $array=array("validity"=>$validity,"prodet_id"=>$prodet_id,"pname"=>$name,"price"=>$price);
    }
    else{
    $array=array("validity"=>$validity,"prodet_id"=>$prodet_id,"pname"=>$name);
}
...
// var_dump($totalQty['qty_item']);
if(isset($totalQty['qty_item'])){
...
// echo json_encode("expresaaaaaaaaaaaaaas");
...
WHERE pId = ? and  tempUser = ?";

$this->dbF->setRow($sqll, array($diff_result[$i], $TempUserId));
...
// $result['status'] = "stringNOTdone";
}else{
// $result['status'] = "stringdone";
//success full add or update

if( ! isset($_POST['free_gift'] ) ){
// $result['status'] = "stringdoneINFOR";
...
$products = "<div class='col9_main iHaveProducts'>$products</div>
...
$rate_star .= '<img src="https://sharkspeed.com/new/webImages/35.png" alt=""> ';

}else if( $rate_avg > $i-0.8){

//$class = "ratings_half";

$rate_star .= ' <img src="https://sharkspeed.com/new/webImages/34.png" alt="">';

}

else {

// $rate_star .= '<img src="https://sharkspeed.com/new/webImages/34a.png" alt="">';
$rate_star .= '';
...
<a href='$product_link'>
<div class='col3_box_img'> <img src='$productImage' alt='$pName'>$saleDivDisVal
...
// foreach ($propri_id_color_name as $val_propri_id) {
...
// $select_box .= "<option value='{$val['prodet_id']}::{$val_prosiz_id['prosiz_id']}--{$val_propri_id['propri_id']}' data-img='{$productImage}'>{$product_name} - {$val_prosiz_id['prosiz_name']}- {$val_propri_id['color_name']} </option>";
...
global $_e;
if(!$this->functions->getFormToken('CheckoutSubscription')){return false;}
...
$sql = "DELETE FROM cart_order_remaining

WHERE userId = ? AND invId <> ?";

$this->dbF->setRow($sql, array($check['orderUser'], $den_invId));
...
$this->dbF->setRow($sql, array($den_invId));
