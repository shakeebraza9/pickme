   ...
   public function get_product_view($cat_id=false){
    ...
    public function pBox($pId, $wishList = false, $view = true)
    {
        ...
        if ($newPrice != $pPrice && $pPriceActual != '') {
            ...
            $oldPriceDiv = '<span class="oldPrice price color_change h3"><del class="">' . $pPriceActual . '</del></span> ';
            $newPriceDiv = '<span class="NewDiscountPrice h2">' . $newPrice . '</span>';
        } else {
            $oldPriceNormal = "";
            $newPriceNormal = $pPrice;
            $oldPriceDiv = "";
            $newPriceDiv = '<span class="NewDiscountPrice price">' . $pPrice . '</span>';
        }
        ...
        $addToCartButton = "
               ...
               <input type='hidden' id='store_$pId' value='$storeId'/>
                ...
                <input type='hidden' id='hidden_stock_$pId' value='$totalQty'/>
...
HTML;
            ...
            $temp = "
        <div class='grid_box_white pBox pFourGrid' data-id='$pId' data-sr='enter bottom and scale up 80% over 1.5s' >
                $wishListRemove
...
HTML;
    ...
    private function productPrint($productIds, $QuickViewId = false, $products = "", $array = array())
    {
        ...
        foreach ($productIds as $p) {
            ...
            if(isset($cat_id) && !empty($cat_id) && $i == 1){
                    ...
                    $products .= "<div class='deal_box' data-id='$id'>
                                    <input type='hidden' class='hidden' value='$price $currencySymbol' id='dealPrice_$id'>
                                    <div class='deal_box_heading'>".$_e['Package Deal']."</div>
                                        <a href='{$link}'>
                                            <div class='deal_box_img'><div class='img_main1'> <img src='{$img}' alt='{$name}'>
                                            <div class='grid_discount_tag'>
                                                <h4>{$dealDiscount}</h4>
                                                <h5>RABATT</h5> 
                                            </div>
                                        ...
                                        <div class='pgrid_text'>
                                            <div> <a href='{$link}' class='pgrid_text_head'>{$name}</a> 
                                            </div>
                                        </div>
                                        <!--pgrid_text end-->
                                        <div class='pgrid_prize'>
                                            <h4>{$priceDiv}</h4>
                                            <h6><span class='oldPrice price color_change h3'><del class=''>{$totalpPrice}</del></span></h6>
                                        </div>
                                    </div>
                                </div>";            
                    }
    ...
    private function sortByProductQuery()
    {
        ...
        switch ($sortBy) {
            case "lowPrice":
				...
				$sql2 = " ORDER BY detail.product_discount_$currencyId ASC";
            ...
            case "highPrice":
				...
				$sql2 = " ORDER BY detail.product_discount_$currencyId DESC";
                break;
            case "lowRate":
    ...
    private function filerByPriceRange($curId){
        ...
        $sql = " AND `detail`.`product_discount_$curId` BETWEEN $minVal AND $maxVal ";
        ...
        if ($this->dbF->rowCount > 0) {
            ...
            <div class='cart_1_inner          tr_{$cartId}'>
                ...
                <div class='cart_1_inner_2'>
                    <h3>{$pName} {$three_for_2_category}</h3>
...
public function getNoProductsCategory($id){
    ...
    public function countPublishProducts($pIds){
        ...
        foreach ($pIds as $key => $value) {
            ...
            $res = $this->dbF->getRow($sql, array($pIds['prodet_id'],'publicAccess','1'));
            if($this->dbF->rowCount){
                $count++;
            }
        }
...
public function getCatDeals($catId, $index){
    $sql = "SELECT id,name,price,image,slug,package FROM product_deal WHERE publish = '1' AND `category` LIKE '%$catId%'";
